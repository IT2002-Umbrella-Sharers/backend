...
def check_result_login(email,password): ##for logging in
    try:
        ...
        if len(res) == 0:
            return False
        return {'user': res[0]} # returns a list containing a single dictionary with user details
    except:
        return False #honestly not sure why this is here but just in case - if you obtain from 
...
def get_locations(): # getting names of current location on login
    try:
        ...
        res = db.execute(statement)
        res = generate_table_return_result(res)
        return res # example: [{'name': 'Bedok MRT'}, {'name': 'Tampines MRT'}, {'name': 'City Hall MRT'},...
    except:
        return False
